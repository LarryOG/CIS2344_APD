...
/**
 * Implements absorbing bounded semaphores.
 * A bounded semaphore has an upper limit for its value as well as
 * the lower limit of zero.
 * A blocking bounded semaphore will only sleep when a process tries
 * to decrease its value below its lower limit (zero).  When a
 * process attempts to increase its value beyond its upper limit the
 * increase will be ignored, and the process will not sleep.
 *
 * @author Hugh Osborne
...
public class AbsorbingSemaphore implements SemaphoreInterface
{
    ...
    public synchronized void vote() throws InterruptedException {
        if (value < limit) { // only increase the value if it has not yet reached the upper limit
